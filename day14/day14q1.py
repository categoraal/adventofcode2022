...
maxX = (max(max(map(max, input)))) #upper x boundary
maxY = max(map(max,[[col[1] for col in row] for row in input]))
minX = (min(map(min, [[col[0] for col in row] for row in input])))
